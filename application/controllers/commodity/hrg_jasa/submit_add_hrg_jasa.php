...
foreach ($post as $key => $value) {
  ...
  if(is_array($value)){
    ...
    foreach ($value as $key2 => $value2) { 
      ...
      //$this->form_validation->set_rules("code_inp[$key2]", "lang:code #$key2", 'required|integer|is_unique[com_srv_price.srv_price_id]|max_length['.DEFAULT_MAXLENGTH_CODE.']');
      ...
      //$input[$key2]['srv_price_id']=url_title($post['code_inp'][$key2],"_",false);
...
if ($this->form_validation->run() == FALSE){
  ...
  foreach ($input as $key => $value) {
    $com = $value['comment'];
    unset($value['comment']);
    $act = $this->Commodity_m->insertDataSrvPrice($value);
    if($act){
      $this->Comment_m->insertCommodity($value['srv_catalog_code'],"SERVICE PRICE",$com,$act,"Buat");
    }
  }
