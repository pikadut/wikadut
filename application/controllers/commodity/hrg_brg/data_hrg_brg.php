...
if(!empty($search)){
    $this->db->group_start();
  $this->db->like("LOWER(mat_price_id)",$search);
  $this->db->or_like("LOWER(A.mat_catalog_code)",$search);
  $this->db->or_like("LOWER(del_point_name)",$search);
  $this->db->or_like("LOWER(A.short_description)",$search);
...
$data['total'] = $this->Commodity_m->getMatPrice()->num_rows();
...
$rows = $this->Commodity_m->getMatPrice()->result_array();
...
foreach ($rows as $key => $value) {
  ...
  $rows[$key]['is_active'] = ($rows[$key]['is_active'] == 1) ? "Aktif" : "Tidak Aktif";
    if(!empty($filtering) && $filtering == "approval"){
    $rows[$key]['operate'] = site_url("commodity/daftar_pekerjaan/approval_hrg_brg/".$rows[$key]["mat_price_id"]);
  }
}
