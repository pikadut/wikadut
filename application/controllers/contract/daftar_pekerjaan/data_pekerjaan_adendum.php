...
$field_order = (isset($get['sort']) && !empty($get['sort'])) ? $get['sort'] : "B.ammend_id";

if(!empty($officer)){
  $this->db->where("cac_pos_code",$userdata['pos_id'],false);
} else {
  $this->db->where("ammend_id","");
}
...
if(!empty($search)){
  ...
  $this->db->or_where("B.ammend_id",$search);
  $this->db->group_end();
}

$data['total'] = $this->Addendum_m->getPekerjaan($id)->num_rows();
...
$rows = $this->Addendum_m->getPekerjaan($id)->result_array();


$status = array(1=>"Belum Disetujui",2=>"Telah Disetujui",3=>"Ditolak");

foreach ($rows as $key => $value) {
  $act = $this->Workflow_m->getActivity($rows[$key]['cac_activity'])->row_array();
