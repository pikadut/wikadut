...
$field_order = (isset($get['sort']) && !empty($get['sort'])) ? $get['sort'] : "B.ptm_number";

if(!empty($userdata['pos_id'])){
  $this->db->where("ccc_pos_code",$userdata['pos_id'],false);
} else {
  $this->db->where("B.ptm_number","");
}
...
$data['total'] = $this->Contract_m->getPekerjaan($id,$userdata['employee_id'])->num_rows();
