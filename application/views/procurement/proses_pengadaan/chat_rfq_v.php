...
<div class="modal fade" id="chatModal" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <!--Modal: Contact form-->
        ...
        <div class="modal-content">
            ...
            <div class="modal-header primary-color white-text">
               <button type="button" class="close waves-effect waves-light" data-dismiss="modal" aria-label="Close">
                ...
                <h4 class="title">
                    <i class="fa fa-pencil"></i> Chatting RFQ</h4>
            </div>
            <!--Body-->
            ...
            overflow-y: auto;">
              ...
              <!-- Material form contact -->
                <form method="POST" id="modalForm" enctype="multipart/form-data" class="ws-validate" action="<?= site_url('procurement/submit_chat_pr')?>" >
                  ...
                  <input type="hidden" name="rfq_number" value="<?= $permintaan['ptm_number']; ?>">
...
<script>
  $(document).ready(function(){
   var table =  $('.table-chat').DataTable({
        ...
        pageLength: 10,
        responsive: true,
        ajax: '<?= site_url('procurement/chat_rfq')."?id=".$permintaan['ptm_number']; ?>',
        "processing": true,
        ...
        "columns": [
            ...
            { "data": "attach", "render": function ( data, type, row, meta ) {
              return data != null ? '<a href="<?= site_url('log/download_attachment/procurement/chat_rfq')."/"?>'+data+ '" target="_blank">'+data+'</a>' : ' - ';
            } 
          }
        ],
      ...
      $('#modalForm').on('submit', function(e){
        ...
        if ($('#id_employee_to').val() == null) {
          ...
          html = "<div class='error' ><span id='helpBlock' class='help-block' style='color:red'>Pesan Harus diisi !!</span></div>";
            ...
            $.ajax( {
              url: '<?= site_url('procurement/submit_chat_rfq')?>',
              type: 'POST',
              data:form_data,
              contentType: false,  
              cache: false,  
