			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<form role="form" id="header" method="POST" action="<?php echo site_url('pengadaan/submit_duatahap') ?>" class="form-horizontal">	
						...
						<div class="form-group" style="display: none"><label class="col-sm-2 control-label"><?php echo $this->lang->line('Tipe Penawaran'); ?></label>
						...
						<div class="form-group"><label class="col-sm-2 control-label"><?php echo $this->lang->line('Lampiran Bidbond'); ?> <small>(Max 2MB)</small></label>
						...
						<div class="form-group"><label class="col-sm-2 control-label"><?php echo $this->lang->line('Kandungan Lokal'); ?></label>
							<div class="col-md-2 m-l-n"><input <?php echo $readonly; echo $hanyabaca; ?> id="kandunganlokal" name="kandunganlokal" placeholder="%" type="number" min="0" max="100" class="form-control" required value="<?php if(isset($header)) { echo $header["pqm_local_content"]; } ?>"></div>
						</div>
						...
						<div class="form-group" id="selesai"><label class="col-sm-2 control-label"><?php echo $this->lang->line('Berlaku Hingga'); ?></label>
							<div class="col-md-4 m-l-n input-group date">
								<span class="input-group-addon"><i class="fa fa-calendar"></i></span><input <?php echo $readonly; echo $hanyabaca; ?> id="berlakuhingga" name="berlakuhingga" type="text" class="form-control" value="<?php if(isset($header)) { echo date("Y-m-d", strtotime($header["pqm_valid_thru"])); } ?>" required>
							</div>
						</div>
						<div class="form-group"><label class="col-sm-2 control-label"><?php echo $this->lang->line('Lampiran Penawaran'); ?> <small>(Max 10MB)</small></label>
							<div class="col-lg-6 m-l-n"><input <?php echo $readonly; echo $hanyabaca; ?> id="lampiran_penawaran" name="lampiran_penawaran" type="file" class="file"><?php if(isset($header)){ ?><a href="<?php echo site_url('pengadaan/download/penawaran/'.$this->umum->forbidden($this->encryption->encrypt($header["pqm_att_quo"]), 'enkrip')); ?>"><?php echo $header["pqm_att_quo"]; ?></a><?php } ?></div>
						</div>
					...
					<form role="form" id="teknis" method="POST" action="<?php echo site_url('pengadaan/submit_duatahap') ?>" class="form-horizontal">	
						...
						<table class="table table-striped">
                            <thead>
								<tr>
									<th style="text-align: center"><?php echo $this->lang->line('Nomor'); ?></th>
									<th style="width: 55%; text-align: center"><?php echo $this->lang->line('Deskripsi'); ?></th>
                            ...
                            <tbody>
									...
									<?php
										if($quos) { 
											...
											<td><?php echo $row["tit_quantity"] ?></td>
											<input type="hidden" id="qty_<?php echo $i ?>_temp" name="price_<?php echo $i ?>" value="<?php echo $row["tit_quantity"] ?>">
											<td><input <?php echo $readonly ?> type="text" class="form-control" id="desc_<?php echo $i ?>" name="desc_<?php echo $i ?>" value ="<?php echo $row["pqi_description"] ?>" required></td>
											<td><input <?php echo $readonly ?> onblur="fnChange('<?php echo $i ?>','qty')" type="text" class="form-control" id="qty_<?php echo $i ?>" name="qty_<?php echo $i ?>" value ="<?php echo $row["pqi_quantity"] ?>" required></td>
											...
											<td><input <?php echo $readonly ?> onblur="fnChange('<?php echo $i ?>','price')" type="text" class="form-control" id="price_<?php echo $i ?>" name="price_<?php echo $i ?>" value ="<?php echo $row["pqi_price"] ?>" required></td>
        ...
        <div class="col-lg-12">
			<div class="ibox float-e-margins">
				<div class="ibox-content text-center">
					<?php if(!isset($readonly) || $readonly != "readonly") { ?>
					<?php if($modes == 'teknis') { ?>
					<button class="btn btn-primary" type="submit" id="submitBtn"><?php echo $this->lang->line('Kirim Penawaran Teknis'); ?></button>
					<?php } else {  ?>
					<button class="btn btn-primary" type="submit" id="submitBtn1"><?php echo $this->lang->line('Kirim Penawaran Harga'); ?></button>
					<?php } ?>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#submitBtn").click(function(){
			//Submit All Form
			if($("#adm").validate().form() && $("#header").validate().form() && $("#teknis").validate().form()){
			button_disabled();
				$("#header").ajaxSubmit({
	...
	function fnChange(id,param){
		...
		if(param == "qty" || param == "price"){
			...
			if(pajak == 1){
				...
				$("#ppnss").text(cur+accounting.formatNumber(subtotal*0.1, 2, ","));
			}
			else{
